...
require('../PDFphp/fpdf/fpdf.php');
	...
	class PDF extends FPDF
	{
	  ...
	  //   $this->Ln(8);//SALTO DE LINEA DE 20 PUNTOS
		...
		// 	$this->Cell(60);
	  //   $this->Cell(60,10,'DATOS PERSONALES ALUMNOS',0,0,'C');
	...
	$id_ci = $_GET['id'];
	$query="SELECT * FROM usuario INNER JOIN tutor ON usuario.ci_tutor=tutor.ci_tutor WHERE usuario.id_ci= $id_ci";
	$alumno=$mysqli->query($query);
	$fila = $alumno->fetch_assoc();

	$idg=$fila['id_grado'];

	$query2="SELECT grado FROM grado WHERE id_grado= $idg";
	$gradoalumno=$mysqli->query($query2);
	$grado= $gradoalumno->fetch_assoc();
	...
	$pdf->Cell(100,6,utf8_decode($grado['grado']),0,0,'L',0);
	$pdf->Ln(10);
	...
	$pdf->SetFont('Arial','B',16);
	//$pdf->SetX(10);
	$pdf->Cell('',9,'DATOS DEL TUTOR',0,0,'C',0);
	$pdf->SetLineWidth(0.6);
	$pdf->Line(82,157,135,157);
	$pdf->Ln(20);
